class Solution:
    ...
    def factorial(self, n):
        ...
        count = []
        primepowerstring = []
        primeflag = [1 for i in range(n+1)]
        primeflag[0] = 0
        primeflag[1] = 0
        temp = 0
        ...
        for i in range(length):
            primecount = 0
            divide = n
            while divide >= prime[i]:
                divide = divide // prime[i]
                primecount += divide
            count.append(primecount)
        # print(prime)
        # print(count)
        for i in range(length):
            # print(str(prime[i]**count[i]))
            primepowerstring.append(str(prime[i]**count[i]))
            # print(i,prime[i],'^',count[i],'=',primepowerstring[i])

        result = '1'
        for i in range(length):
            result = self.multiply(result,primepowerstring[i])
        print(result)
        return result
